...
void main() {
  group('FinantialHealthBloc', () {
    blocTest('emits [FinantialHealthTest] when FinantialHealthStart Event added',
    ...
    blocTest('emits [FinantialHealthFinished] when GameTimeOut Event added',
        build: () => FinantialHealthBloc(), act: (bloc) => bloc.add(CalculateFinantialHealthScore(annualIncome: 1000, monthlyCosts: 10)), expect: () => [isA<FinantialHealthFinished>()]);

    blocTest('emits [FinantialHealthFinished] when GameTimeOut Event added',
        build: () => FinantialHealthBloc(), act: (bloc) => bloc.add(CalculateFinantialHealthScore(annualIncome: 1000, monthlyCosts: 80)), expect: () => [isA<FinantialHealthFinished>()]);
  });
}
