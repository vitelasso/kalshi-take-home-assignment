...
class FinantialScoreCardWidget extends StatelessWidget {
  ...
  const FinantialScoreCardWidget({Key? key, required this.userFinantialStatus}) : super(key: key);
            ...
            child: Padding(
              ...
              child: Column(
                children: [
                  ...
                  SvgPicture.asset("assets/${userFinantialStatus.score.toString().toLowerCase()}.svg"),
                  const SizedBox(height: 24),
                  Text(
                      Constants.strFinantialWellnessScoreTitleMap[userFinantialStatus.score.toString()] ?? 'Unknown',
                    style: TextStyle(color: Theme.of(context).primaryColor, fontSize: 20, fontWeight: FontWeight.bold),
                  ),
                  const SizedBox(height: 8),
                  Text(
                      Constants.strFinantialWellnessScoreSubTitleMap[userFinantialStatus.score.toString()] ?? 'Unknown',
              ...
              child: ElevatedButton(
                      onPressed: () => BlocProvider.of<FinantialHealthBloc>(context).add(FinantialHealthStart()),
                      style: ButtonStyle(
